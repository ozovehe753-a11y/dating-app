...
export default function Login() {
  ...
  return <main className="authPage"><form className="authCard" onSubmit={submit}><a className="logo" href="/">♥ Heartly</a><h1>Welcome back</h1><p>Log in to continue discovering connections.</p>{error && <div className="error">{error}</div>}<label>Email<input name="email" type="email" required /></label><label>Password<input name="password" type="password" required /></label><button className="large" type="submit">Log in</button><p className="authSwitch">New to Heartly? <a href="/signup">Create an account</a></p></form></main>;
}
